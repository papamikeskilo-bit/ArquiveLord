...
void HTMLWindow::Create(std::string const& title) {
  ...
  hwnd_ = ::CreateWindowExA(0, "subkdls490", title.c_str(), WS_OVERLAPPEDWINDOW,
            CW_USEDEFAULT, 0, 
            CW_USEDEFAULT, 0,
            HWND_DESKTOP , 0, 
            instance_    , 0);
  ...
  if (hwnd_) { 
    SetWindowLongA(hwnd_, GWL_USERDATA, (LONG)this);
    if (CreateBrowserObject()) {
      ::MessageBoxA(0, "Could not create browser object", 0, 0);
    }
...
#define OLERENDER_DRAW 1
  ...
  if (! ::OleCreate(
    ...
    ::MessageBoxA(0, "Something went wrong -3", 0, 0);
    /* Something went wrong!
     TODO  UnEmbedBrowserObject(hwnd_); */
    return(-3);
  }

  ::MessageBoxA(0, "Something went wrong -2", 0, 0);
...
void HTMLWindow::Resize(DWORD width, DWORD height) {
  ...
  if (!browserObject_->QueryInterface(IID_IWebBrowser2, (void**)&webBrowser2)) {
    ...
    ::MessageBoxA(0, "Couldn't query interface for IID_IWebBrowser2", 0, 0);
  }
}
...
//#define DISPID_PROGRESSCHANGE     108   // sent when download progress is updated
  ...
  case DISPID_BEFORENAVIGATE     :   // this is sent before navigation to give a chance to abort
    ::MessageBoxA(0, "Before Navigate", 0, 0);
    return S_OK;
    break;
...
//  break;
  ...
  case DISPID_NAVIGATEERROR: {   // Fired to indicate the a binding error has occured
    ...
    sprintf_s(buf, 255, "Navigate Error, code is: %d", static_cast<int>(StatusCode.lVal));
  ...
  default:
	  char buf[256] = {0};
    sprintf_s(buf, 255, "invoke, dispid: %d", static_cast<int>(dispIdMember));
    if (TODO_bool) ::MessageBoxA(0, buf, 0, 0);
...
void HTMLWindow::AddSink() {
  ...
  if (!browserObject_) {
    ::MessageBoxA(0, "browserObject_ is null in AddSink", 0, 0);
    return;
  }
  if (browserObject_->QueryInterface(IID_IConnectionPointContainer, reinterpret_cast<void**>(&cpc)) == S_OK) {
    if (cpc->FindConnectionPoint(DIID_DWebBrowserEvents2, &cp) == S_OK) {
      ...
      if (! (SUCCEEDED(cp->Advise(static_cast<IDispatch*>(this), &cookie)) ) ) {
        ::MessageBoxA(0, "Advise failed", 0, 0);
      }
    }
    else {
      ::MessageBoxA(0, "FindConnectionPoint", 0, 0);
    }
  }
  else {
    ::MessageBoxA(0, "QueryInterface for IID_IConnectionPointContainer", 0, 0);
  }
}
        ...
        if (win) {
          if (!win->browserObject_->QueryInterface(IID_IWebBrowser2, (void**)&webBrowser2)) {
            ...
            if (ipao) {
              ...
              ::MessageBoxA(0, "Failed to retrieve IOleInPlaceActiveObject in WM_KEYDOWN", 0, 0);
            }
          }
          return 0;
        }
        else {
          ::MessageBoxA(0, "Failed to retrieve webBrowser2 in WM_KEYDOWN", 0, 0);
        }
        return -1;
      }
      break;
